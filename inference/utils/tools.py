...
class Discretizer:
    ...
    def discretize(self, context, fit_length=None, flag="test"):
        if flag == "train":
            series = context
            # hist_series
            context = series[0]
            ...
            hist_dispersed_context = dispersed_context
            ...
            dispersed_context[np.isnan(context)] = np.nan
            pred_dispersed_context = dispersed_context

            return hist_dispersed_context, pred_dispersed_context
...
def int_list_to_string(int_list, separator=" "):
    """
    将整数列表转换为字符串
    ...
    """
    # 检查列表是否包含NaN值
    if isinstance(int_list, np.ndarray) and np.isnan(int_list).any():
        # 处理包含NaN的numpy数组
        return separator.join(str(x) if not np.isnan(x) else "NaN" for x in int_list)
    else:
        # 处理普通列表或没有NaN的numpy数组
        return separator.join(str(x) for x in int_list)
