...
if __name__ == "__main__":
    ...
    index = int(len(timey) * test_sample_ratio)
    timey = timey[index:]
    step = 3
    timey = sample_data_by_step(timey, step)

    # predict data
    # pred_file = "./token_pred_result/few-shot/ChatTime/remap_itself/" \
    #             f"sample{sample_ratio}/test_pred_y_codes_{dataset_name}.npy"
    # pred_file = "./token_pred_result/few-shot/ChatTime/remap_3/" \
    #             f"sample{sample_ratio}/test_pred_y_codes_{dataset_name}.npy"
    pred_file = "./token_pred_result/few-shot/ChatTime/remap_itself_0.2test/" \
                f"sample{sample_ratio}/test_pred_y_codes_{dataset_name}.npy"
    pred_code_ids = np.load(pred_file)# [bs * nvars, compressed_time]
    ...
    # save_path = "./token_pred_result/few-shot/ChatTime/remap_itself/" \
    #             f"sample{sample_ratio}/test_pred_y_result_{dataset_name}.npy"
    # save_path = "./token_pred_result/few-shot/ChatTime/remap_3/" \
    #             f"sample{sample_ratio}/test_pred_y_result_{dataset_name}.npy"
    save_path = "./token_pred_result/few-shot/ChatTime/remap_itself_0.2test/" \
                f"sample{sample_ratio}/test_pred_y_result_{dataset_name}.npy"
    np.save(save_path, pred_y_arr)
